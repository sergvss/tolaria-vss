...
- **Rust coverage в `pnpm test:coverage`** требует `rustup component add llvm-tools-preview`. Шаг `pnpm install` его не добавляет автоматически.
...
- **Code signing** для NSIS-инсталлятора не настроен. Tauri auto-updater работает только для подписанных билдов, поэтому обновления из самостоятельно собранного инсталлятора апдейтером приложения подхвачены не будут.
...
- Приложение запускается, открывает дефолтный vault (`~/Laputa` или `%USERPROFILE%\Laputa`)
